...
def dir_brute_force(url, wordlist_path='/work/backend/core/enum/resources/common.txt'):
    """
    Brute-forces directories using the wordlist and saves the results to the database.
    """
    domain = urlparse(url).hostname
    output_dir = f'/work/backend/core/output/{domain}'
    out_file = f"{output_dir}/dir_brute.json"
    ...
    print(f"\n[*] Starting Directory Brute Force at {url}")
    ...
    try:
        with open(out_file, 'r') as f:
            ...
            for result in results:
                ...
                if status_code in [301, 302, 303, 307, 308] and location_header:
                    print(f"  -> {full_url} [Status: {status_code}, Length: {content_length}] -> Redirects to: {location_header}")
                else:
        ...
        print(f"\n[+] Finished Directory Brute Force at {url}")
