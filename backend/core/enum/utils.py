...
def load_urls_for_mining(domain):
    """
    Loads URLs that is HTML & 2xx,3xx status code for a domain and saves to htmlurls.txt
    """
    out_file = f'/work/backend/core/enum/output/{domain}/htmlurls.txt'
    ...
    urls = URL.objects.filter(
        ...
        status_code__range=(400, 599)
    )

    req_urls = set()

    for u in urls:
        if 300 <= u.status_code < 400:
            if u.location == f"{u.endpoint}/" or u.location == u.endpoint.rstrip('/'):
                req_urls.add(u.location)
        else:
